...
class BaseCountdown {
    ...
    private String mCodeText = UIUtil.getString(R.string.code_mobile);
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, UIUtil.sp2px(mContext, 12));
        ...
        isShowMinute = ta.getBoolean(R.styleable.CountdownView_isShowMinute, false);
        isShowSecond = ta.getBoolean(R.styleable.CountdownView_isShowSecond, false);
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, UIUtil.sp2px(mContext, 12));
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = (int) UIUtil.dp2px(DEFAULT_SUFFIX_LR_MARGIN);
    ...
    private float getDayAndHourContentWidth() {
        ...
        if (isShowDay) {
            String tempDay = StringUtil.formatNum(mDay);
        ...
        if (isShowHour) {
            String tempHour = StringUtil.formatNum(mHour);
    ...
    public void onMeasure(View v, int viewWidth, int viewHeight, int allContentWidth, int allContentHeight) {
        ...
        //修改次满足验证码功能,适配宽高
        if (null!=mSuffixSecond&&mSuffixSecond.equals(mCodeText))
         mLeftPaddingSize=0;
        else
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(StringUtil.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            canvas.drawText(StringUtil.formatNum(mHour), mHourLeft + curTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(StringUtil.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2 , mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowSecond) {
            // draw second text
                //修改当需要更改文字，给文字让出空间
                if (null!=mSuffixSecond&&mSuffixSecond.equals(mCodeText))
                    canvas.drawText("", mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
                else
                    canvas.drawText(StringUtil.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(StringUtil.formatMillisecond(mMillisecond), mMillisecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = UIUtil.sp2px(mContext, textSize);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = UIUtil.sp2px(mContext, textSize);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = UIUtil.dp2px( suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = UIUtil.dp2px( suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = UIUtil.dp2px( suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = UIUtil.dp2px(suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = UIUtil.dp2px(suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = UIUtil.dp2px(suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = UIUtil.dp2px( suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = UIUtil.dp2px( suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = UIUtil.dp2px( suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = UIUtil.dp2px( suffixMillisecondMarginL);
            isRef = true;
        }
