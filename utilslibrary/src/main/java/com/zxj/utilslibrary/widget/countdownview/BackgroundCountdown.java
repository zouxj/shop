...
class BackgroundCountdown extends BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeBgDivisionLineSize = ta.getDimension(R.styleable.CountdownView_timeBgDivisionLineSize, UIUtil.dp2px(DEFAULT_TIME_BG_DIVISION_LINE_SIZE));
        mTimeBgSize = ta.getDimension(R.styleable.CountdownView_timeBgSize, 0);
        mDefSetTimeBgSize = mTimeBgSize;
        mTimeBgBorderSize = ta.getDimension(R.styleable.CountdownView_timeBgBorderSize, UIUtil.dp2px( DEFAULT_TIME_BG_BORDER_SIZE));
    ...
    protected void initTimeTextBaseInfo() {
        ...
        if (mDefSetTimeBgSize == 0 || mTimeBgSize < mTimeTextWidth) {
            mTimeBgSize = mTimeTextWidth + (UIUtil.dp2px( 2) * 4);
        }
    }
    ...
    private void initHasBackgroundTextBaseY(RectF rectF) {
        ...
        mTimeBgDivisionLineYPos = rectF.centerY() + (mTimeBgDivisionLineSize == UIUtil.dp2px(DEFAULT_TIME_BG_DIVISION_LINE_SIZE) ? mTimeBgDivisionLineSize : mTimeBgDivisionLineSize / 2);
    }
    ...
    public int getAllContentWidth() {
        ...
        if (isShowDay) {
            if (isDayLargeNinetyNine) {
                ...
                mDayTimeBgWidth = rect.width() + (UIUtil.dp2px( 2) * 4);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            ...
            // draw day text
            canvas.drawText(StringUtil.formatNum(mDay), mDayBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            // draw hour text
            canvas.drawText(StringUtil.formatNum(mHour), mHourBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowMinute) {
            ...
            // draw minute text
            canvas.drawText(StringUtil.formatNum(mMinute), mMinuteBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowSecond) {
            ...
            // draw second text StringUtil.formatNum(mSecond)
            canvas.drawText(StringUtil.formatNum(mSecond), mSecondBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(StringUtil.formatMillisecond(mMillisecond), mMillisecondBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
    ...
    public void setTimeBgRadius(float radius) {
        mTimeBgRadius = UIUtil.dp2px( radius);
    }
    ...
    public void setTimeBgDivisionLineSize(float size) {
        mTimeBgDivisionLineSize = UIUtil.dp2px(size);
        if (null != mTimeBgDivisionLinePaint) {
            mTimeBgDivisionLinePaint.setStrokeWidth(mTimeBgDivisionLineSize);
        }
    }
    ...
    public void setTimeBgBorderSize(float size) {
        mTimeBgBorderSize = UIUtil.dp2px( size);
    ...
    public void setTimeBgBorderRadius(float size) {
        mTimeBgBorderRadius = UIUtil.dp2px( size);
    }
