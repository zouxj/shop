...
public class LineChartView extends View {
    ...
    private int rulerValueDefault = 0;
...
//        } while (startValue < endValue);
        for (int i=-6;i<=6;i+=2) {
            int startHeight = -getValueHeight(i);
            tablePath.moveTo(stepStart, startHeight);
            tablePath.lineTo(tableEnd, startHeight);
            drawRulerYText(canvas, String.valueOf(Math.abs(i)), stepStart, startHeight);
        }
    ...
    private void drawRulerXValue(Canvas canvas) {
        ...
        for (int i = 0; i < linePoints.length; i++) {
            ...
            if (point == null) break;
            drawRulerXText(canvas, "0"+String.valueOf(i), linePoints[i].x, 0);
        }
    }
